...
import 'dtos/event_cordinates/event_cordinates.dart';
...
class IEventRepository extends EventRepository {
  ...
  Future<List<EventCordinates>> getAllEventsCoordinate() async {
    try {
      String? token = await FirebaseAuth.instance.currentUser!.getIdToken(true);
      final url = '$serverUrl${EventApiConstants.EVENTS_ALL}';
      final response = await RESTService.performGETRequest(
      ...
      print(eventsRaw);
      final events = eventsRaw.map((e) => EventCordinates.fromJson(e)).toList();
      return events;
  ...
  Future<PaymentStatusDto> fetchPaymentStatusByIdForCover(
    ...
    try {
      final url =
          '$serverUrl${BookingApiConstants.BOOKING_STATUS}/$bookingId/cover/$coverId';
      String? token = await FirebaseAuth.instance.currentUser!.getIdToken(true);
